...
void	put_block(t_mlx *MLX, unsigned long int x, unsigned long int y, char c)
{
	if (c == '1')
		...
		mlx_put_image_to_window(MLX->mlx, MLX->win, MLX->images->player,
			x + MLX->img_location[0], y + MLX->img_location[1]);
	else if (c == 'E')
		mlx_put_image_to_window(MLX->mlx, MLX->win, MLX->images->door,
			x + MLX->img_location[0], y + MLX->img_location[1]);
	else if (c == 'C')
		mlx_put_image_to_window(MLX->mlx, MLX->win, MLX->images->collectible,
			x + MLX->img_location[0], y + MLX->img_location[1]);
	else if (c == 'W')
		mlx_put_image_to_window(MLX->mlx, MLX->win, MLX->images->win,
			x + MLX->img_location[0], y + MLX->img_location[1]);
	else if (c == 'F')
		mlx_put_image_to_window(MLX->mlx, MLX->win, MLX->images->enemy,
			x + MLX->img_location[0], y + MLX->img_location[1]);
}
